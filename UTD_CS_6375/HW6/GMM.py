...
class GMM(object):
    ...
    def get_initial_centers(self, X):
        ...
        for k in range(K):
            centers[k] = np.array(np.random.choice(np.arange(-3, 4, 1), N)).reshape(1, N)
        return centers
    ...
    def fit(self, X):
        ...
        return (means, covariances, lambdas, current_loglikelihood, posteriors)
...
for i in range(1):
    for j in range(len(k_list)):
        ...
        (means, covariances, lambdas, current_loglikelihood, posteriors) = gmm.fit(X)
